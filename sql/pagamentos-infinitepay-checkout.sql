-- Checkout Integrado InfinitePay (link dinâmico + confirmação automática)
-- Rodar no SQL Editor do Supabase após pagamentos-cartao-infinitepay.sql
--
...
--
-- No app InfinitePay (computador): Checkout → Checkout Integrado → habilitar
-- "checkout externo" / link integrado (erro external_checkout_not_enabled se desligado).
--
-- Secrets opcionais (Dashboard → Edge Functions → Secrets):
--   COR_SITE_URL = https://corjovem.geucaristica.com.br
...
-- Confirma pagamento aprovado (webhook / sync). Apenas service_role.
...
-- Atualiza consulta pública com dados do checkout
...
-- Config padrão: checkout integrado com handle nicolegiagio
update public.config_camisa_pix
set infinitepay_handle = 'nicolegiagio',
    infinitepay_habilitado = true
where coalesce(nullif(btrim(infinitepay_handle), ''), '') = '';
